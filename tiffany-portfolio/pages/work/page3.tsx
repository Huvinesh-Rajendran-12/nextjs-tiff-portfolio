...
import { getPage3Data, getFontData } from '../api/api';
import { useEffect } from 'react';

import { Entry } from 'contentful';
import { TypeFont, TypePages } from '../../interfaces/contentful';
type PageThreeProps = {
  pageThreeData: Entry<TypePages>
...
export default function PageThree({pageThreeData,fontAPIData}:PageThreeProps) {
  ...
  const pageThreeMetadata = pageThreeData.fields
  useEffect(() => {
    ...
    <div className='flex flex-col'>
      ...
      <section className='text-2xl text-center justify-center flex flex-row text-white pt-2'>
        <div className='flex flex-col gap-y-8'>
          <p className='lg:max-w-xl sm:max-w-sm font-content md:text-lg sm:text-sm'>
              {pageThreeMetadata.pageDescription?.content[0].value}{' '}
          </p>
          <div className='text-white text-center justify-center'>
            <div className='font-content md:text-lg sm:text-sm'>Client: {pageThreeMetadata.projectClient}</div>
            <div className='font-content md:text-lg sm:text-sm'>Designer: {pageThreeMetadata.projectDesigner}</div>
            <div className='font-content md:text-lg sm:text-sm'>Developer: {pageThreeMetadata.projectDeveloper}</div>
          </div>
        </div>
      </section>
      <section className='flex flex-row justify-center mx-20 lg:mx-80'>
        <div className='py-20 space-y-10'>
           <img
            alt=""
            src={
              pageThreeMetadata.pic1?.fields.file.url
            }
            className='w-full block ml-auto mr-auto'
            />
          <div className='flex'>
              <div className='w-1/2 p-1'>
                <img alt='' src={pageThreeMetadata.pic2?.fields.file.url} className='h-full'/>
              </div>
              <div className='w-1/2 p-1'>
                <img alt='' src={pageThreeMetadata.pic3?.fields.file.url} className='h-full'/>
              </div>
            </div>
          ...
          <img
            alt=""
            src={
              pageThreeMetadata.pic4?.fields.file.url
            }
            className='w-full block ml-auto mr-auto'
            />
          <div className='flex'>
              <div className='w-1/2 p-1'>
                <img alt='' src={pageThreeMetadata.pic5?.fields.file.url} className='h-full'/>
              </div>
              <div className='w-1/2 p-1'>
                <img alt='' src={pageThreeMetadata.pic6?.fields.file.url} className='h-full'/>
              </div>
            </div>
        </div>
      </section>
      <section className='relative h-[90vh]'>
            ...
            <p className='whitespace-nowrap uppercase font-medium text-[15vw] md:text-[20vw] pb-8 lg:text-xxl1 leading-1 lg:leading-[15.28vw]'> 
              <span className='text-scrolling font-animation'>{pageThreeMetadata?.animation1}</span>
              <span className='text-scrolling font-animation'>{pageThreeMetadata?.animation1}</span>
            </p>
            <p className='whitespace-nowrap uppercase font-medium text-[15vw] md:text-[20vw] lg:mt-16 lg:text-xxl1 leading-1 lg:leading-[15.28vw]'>
              <span className='reverse text-scrolling font-animation'>{pageThreeMetadata?.animation2}</span>
              <span></span>
            </p>
            </div>
             <div className='absolute top-10 left-0 flex flex-row justify-center w-full h-full'>
              <div className='absolute md:max-w-lg sm:max-w-sm sm:mx-10'>
                <img alt='' src={pageThreeMetadata?.nextPagePic?.fields.file.url} className=' h-[80vh] max-h-200px' />
                  <div
...
export const getStaticProps = async () => {
  const pageThreeData = (await getPage3Data())
